...
async def main():
	...
	async for message in events:
		...
		if schemad:
			...
			print(f"{message['data']['userName']} is watching {message['data']['nowPlaying']['name']} ({message['data']['playState']['positionTicksFormatted']}/{message['data']['nowPlaying']['totalTicksFormatted']})")
		else:
