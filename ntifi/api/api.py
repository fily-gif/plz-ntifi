...
load_dotenv()
server: str = (s := os.getenv("server")) and (
    s if s.startswith(("http://", "https://")) else f"https://{s}"
)
...
ws_server = (
    ...
    + f"/socket?api_key={api_key}&device_id=fjapi-ws"
)
...
class Jellyfin:
    ...
    def __init__(self, creds: dict | None):
        self.creds = (
            creds or None
        )  # optional credentials from an already logged-in state,
    ...
    def auth(self, token: str | None, username=None, password=None):
        ...
        self.sess.headers.update(
            {
                "User-Agent": "fily-github-com/1.0",
                "Authorization": f'MediaBrowser Client="{token or None}", Device="Python", DeviceId="PyJfinApi", Version="10.11.6"',
            }
        )
        ...
        server_is_valid: bool = (
            self.sess.get(f"{server}/System/Info/Public").status_code == 200
        )
        if server_is_valid:
            if token and not (username and password):
                print(
                    f"using token to log in for {server}!"
                )  # FIXME: change all prints to logger!!
                try:
                    login = self.sess.get(
                        f"{server}/System/Info"
                    )  # GETing an authed endpoint
                    if login:  # did we pass the test?
                        print("logged in!")  # TODO: print who we are?
                        return (
                            token,
                            login.json(),
                        )  # returning token for future reference, as well as json for debugging/other info (system/info has some useful stuff!)
                ...
                if login.status_code == 200:
                    ...
                    return login.json()[
                        "AccessToken"
                    ]  # because we're feeling nice today
        else:
            # uh oh!
