...
def main(server):
	with connect(server) as ws:
		ws.send(json.dumps({"MessageType": "SessionsStart", "Data": "0,1000" })) # subscribing to SessionsStart with initialdelay 0, interval 1000
		# ...this means that it sends as soon as possible (0), and then "polls" every 1000ms (god i hate jellyfin api)
		while True:
			message = ws.recv() # wait for message
			print(f"\nReceived message: {message}\n") # got it!!
			if "Episode" in message:
				with open("out.json", "w") as f:
					json.dump(message, f, indent=2, ensure_ascii=True)
